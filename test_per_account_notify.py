...
class TestAppriseTestNotifiesAllNotifiers:
    ...
    def test_main_apprise_test_notifies_global_and_each_account(self):
        recorder = []  # locks the ORDER: global notify -> account notify -> finish_run -> SystemExit
        ...
        mock_cfg.format_date = lambda d: str(d)
        mock_cfg.history.finish_run.side_effect = lambda *a, **kw: recorder.append(f"finish_run:{a[0]}")
        ...
        # History run is still finalized as "apprise_test" before exiting
        mock_cfg.history.finish_run.assert_called_once_with("apprise_test")

        # Locked ORDER: global -> per-account -> finish_run -> SystemExit
        assert recorder == [
            "global_notify",
            "account_notify:chris@example.com",
            "finish_run:apprise_test",
            "SystemExit",
        ]
    ...
    def test_apprise_test_does_not_fire_with_no_notifiers_anywhere(self):
        ...
        # finish_run("ok") fires at the very end of a normal run, but the
        # apprise_test self-test path (which finishes as "apprise_test") must
        # never have been entered.
        finish_run_args = [c.args for c in mock_cfg.history.finish_run.call_args_list]
        assert ("apprise_test",) not in finish_run_args
